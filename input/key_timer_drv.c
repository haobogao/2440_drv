...
void shake_handler_func(unsigned long data)
{
	...
	input_event(dev, EV_KEY, dev->pin[dev->current_key].pin_val, 1);
	input_sync(dev);
...
void set_inputkey_gpio(void)
{
	...
	key_input_dev.pin[0].stat =KEY_STAT_UP;
	...
	key_input_dev.pin[1].stat =KEY_STAT_UP;
	...
	key_input_dev.pin[2].stat =KEY_STAT_UP;
	...
	key_input_dev.pin[3].stat =KEY_STAT_UP;
	key_input_dev.pin[3].irq = IRQ_EINT19;
	strcpy(key_input_dev.pin[3].name,"key3");
}

void __init key_input_init(void)
{
	int i,ret;
	key_input_dev =kzalloc(sizeof(struct key_inpput_dev_t)*count_button, GFP_KERNEL);
	...
	for(i = 0;i <count_button;i++)
		ret =request_irq(key_input_dev[i].pin.irq, handler_button, IRQT_FALLING,key_input_dev[i].pin.name,(void*) (key_input_dev));
...
void __exit key_input_exit(void)
{
	...
	del_timer(&(key_input_dev->shake_handler));
	 for(i = 0;i <count_button;i++)
	 	ret =free_irq(key_input_dev->pin.irq,(void*) (key_input_dev));
