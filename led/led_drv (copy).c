...
{
	char buf[size+1];
	copy_from_user(buf, buff, size+1);
...
void led_setup_cdev(struct led_dev_t * led_dev,int index)
{
	...
	minor_str[0] = '0'+index;
	minor_str[1] = '\0';
	dev_num = MKDEV(major,minor+index);
	printk(KERN_INFO"minor_str:%s\ndevname:%s\n",minor_str,devname);
	//leddev = class_device_create(chartest, NULL,dev_num, NULL,"led%d",index);
	leddev = class_device_create(chartest, NULL,dev_num, NULL,strncat(devname,minor_str,2),0);
